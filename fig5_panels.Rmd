...
pca1 <- fviz_pca_biplot(pca_res, geom.ind="point", axes=c(1,2),
                ...
                pointshape = 19, pointsize=2.25, title="Trametinib sensitivity")  +
...
Look at trametinib effects
```{r}
drug_sensitivity %>% 
  filter(Drug == "Trametinib" & Program %in% emt_arch_high) %>% 
...
scoreCells <- function(cell_line, gene_set){
  ...
  seurat_subset <- AddModuleScore(seurat_subset, features = list(gene_set),
                                  name = "EMP_Sig")
  colnames(seurat_subset@meta.data)[32] <- "EMP_Sig"
...
cowplot::save_plot(drug_plots, filename="trametinib_sample_lines.pdf",
                   base_width = 4.2, base_height=4.5)
```
...
scoreCells <- function(cell_line, gene_set){
  ...
  seurat_subset <- AddModuleScore(seurat_subset, features = list(gene_set),
                                  name = "EMP_Sig")
  colnames(seurat_subset@meta.data)[24] <- "EMP_Sig"
